...
def compute_lighting(hit, ray, light, surfaces, hit_object_diffuse, hit_object_specular, alpha):
    Ld=light.position-hit.hit_point
    if(light_intersect(Ray(hit.hit_point, Ld), surfaces)):
        #TODO add calculation according to correct shadowing
    ...
    specular_color = light.specular_intensity*np.power(np.dot(reflected, -ray.direction), alpha)*np.array(light.color)*np.array(hit_object_specular)
...
def main():
    ...
    for x in range(w):
        for y in range(h):
            ...
            image[x,y]=ray_trace(hit, ray, scene_settings, lights, surfaces, materials, scene_settings.max_recursions)
    image = image*255
